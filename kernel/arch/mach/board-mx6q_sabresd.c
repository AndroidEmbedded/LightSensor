...
#if defined(CONFIG_GPIO_WATCHDOG_DEV)
#include <linux/gpiowatchdog.h>
#endif
...
#define MX6DL_PAD_CSIO_RESET IMX_GPIO_NR(7, 12)
#define MX6DL_PAD_CSIO_PWRDWN IMX_GPIO_NR(4, 5)
#define MX6DL_CVBS_MHL_SWITCH IMX_GPIO_NR(3, 27)
...
#define MHL_INT_N               IMX_GPIO_NR(1, 0)
...
#define SABRESD_GPIO_WATCHDOG  IMX_GPIO_NR(4, 29)
#define SABRESD_ROTARY_KEY	IMX_GPIO_NR(7, 7)
#define SABRESD_ROTARY_LEFT	IMX_GPIO_NR(7, 2)
#define SABRESD_ROTARY_RIGHT	IMX_GPIO_NR(7, 3)
#define SABRESD_ROTARY_COMMON	IMX_GPIO_NR(7, 4)
#define SABRESD_WL_REG_ON	IMX_GPIO_NR(1, 18)
#define SABRESD_WL_HOST_WAKE	IMX_GPIO_NR(1, 20)
#define SABRESD_BT_REG_ON	IMX_GPIO_NR(1, 16)
#define SABRESD_BT_HOST_WAKE	IMX_GPIO_NR(1, 17)
#define SABRESD_BT_WAKE		IMX_GPIO_NR(1, 19)
#define SABRESD_BT_KEY_ROW4_GPIO		IMX_GPIO_NR(4, 15)
...
static void mx6_csi0_tvin_io_init(void)
{
    ...
    gpio_direction_output(MX6DL_PAD_CSIO_PWRDWN, 1);
    msleep(1);
	gpio_direction_output(MX6DL_PAD_CSIO_RESET, 1);
...
#if 0  // remove unused i2c devices
	{
...
#endif
    {
        //I2C_BOARD_INFO("EP957", 0x52>>1), // 0x29
        I2C_BOARD_INFO("EP957", 0x50>>1), // 0x28
        .irq = gpio_to_irq(MHL_INT_N),
    },
};
...
static iomux_v3_cfg_t mx6dl_uart5_cts_pads[] = {
	MX6DL_PAD_KEY_ROW4__UART5_CTS,
};

static iomux_v3_cfg_t mx6dl_uart5_key_row4_gpio_pads[] = {
...
static void mx6q_sd_bt_reset(void)
{
	...
	gpio_direction_output(SABRESD_BT_REG_ON, 0);
	gpio_direction_output(SABRESD_BT_WAKE, 0);
	msleep(20);

	if (cpu_is_mx6dl()) {
		mxc_iomux_v3_setup_multiple_pads(mx6dl_uart5_key_row4_gpio_pads,
				ARRAY_SIZE(mx6dl_uart5_key_row4_gpio_pads));
	}

	gpio_request(SABRESD_BT_KEY_ROW4_GPIO, "bt_row4_gpio");
	gpio_direction_output(SABRESD_BT_KEY_ROW4_GPIO, 0);
	msleep(20);

	gpio_direction_output(SABRESD_BT_REG_ON, 1);
	gpio_direction_output(SABRESD_BT_WAKE, 1);
	msleep(20);

	gpio_free(SABRESD_BT_KEY_ROW4_GPIO);

	if (cpu_is_mx6dl()) {
		mxc_iomux_v3_setup_multiple_pads(mx6dl_uart5_cts_pads,
				ARRAY_SIZE(mx6dl_uart5_cts_pads));
	}

	msleep(50);
}

static void mx6q_sd_bt_off(void)
{
	printk(KERN_INFO "%s()\n", __func__);
	gpio_direction_output(SABRESD_BT_WAKE, 0);
	gpio_direction_output(SABRESD_BT_REG_ON, 0);
...
{
	printk(KERN_INFO "%s(), status=%d(%s)\n", __func__, status, (status)?"ON":"OFF");
...
#ifdef CONFIG_GPIO_WATCHDOG_DEV
struct gpio_watchdog_platform_data gpio_watchdog_info = {
	.gpio_watchdog_gpio = SABRESD_GPIO_WATCHDOG,
};

static struct platform_device gpio_watchdog_device = {
    .name ="gpio-watchdog",
    .dev = {
        .platform_data = &gpio_watchdog_info,
    },
};
#endif
...
static struct rotary_encoder_platform_data raumfeld_rotary_encoder_info = {
    ...
    .axis       = ABS_X,
    .relative_axis  = 0,
    .gpio_a     = SABRESD_ROTARY_RIGHT,
    .gpio_b     = SABRESD_ROTARY_LEFT,
    .inverted_a = 0,
    .inverted_b = 0,
    .rollover	= false,
    .half_period = false,
    .gpio_key_a = KEY_RIGHT,
    .gpio_key_b = KEY_LEFT,
    .common_gpio = SABRESD_ROTARY_COMMON,
    .desc	= "rotary-encoder",
    .gpio = SABRESD_ROTARY_KEY,
...
static iomux_v3_cfg_t mx6dl_uart5_rxd_txd_rts_pads[] = {
...
static void __init uart5_init(void)
{
	...
	if (cpu_is_mx6dl()) {
		mxc_iomux_v3_setup_multiple_pads(mx6dl_uart5_rxd_txd_rts_pads,
				ARRAY_SIZE(mx6dl_uart5_rxd_txd_rts_pads));
		mxc_iomux_v3_setup_multiple_pads(mx6dl_uart5_cts_pads,
				ARRAY_SIZE(mx6dl_uart5_cts_pads));
	}
...
static void __init mx6_sabresd_board_init(void)
{
...
#ifdef CONFIG_GPIO_WATCHDOG_DEV
    platform_device_register(&gpio_watchdog_device);
#endif
	...
	/*add wifi support*/
	if (cpu_is_mx6dl()) {
		mxc_iomux_v3_setup_multiple_pads(mx6dl_wl_bt_pads,
						ARRAY_SIZE(mx6dl_wl_bt_pads));

		/* AMPAK AP6210 WiFi 802.11b/g/n + BT4.0 SIP Module */
		gpio_request(SABRESD_WL_REG_ON, "wl_reg_on");
		gpio_direction_output(SABRESD_WL_REG_ON, 0);

		gpio_request(SABRESD_WL_HOST_WAKE, "wl_host_wake");
		gpio_direction_input(SABRESD_WL_HOST_WAKE);

		gpio_request(SABRESD_BT_REG_ON, "bt_reg_on");
		gpio_direction_output(SABRESD_BT_REG_ON, 0);

		gpio_request(SABRESD_BT_HOST_WAKE, "bt_host_wake");
		gpio_direction_input(SABRESD_BT_HOST_WAKE);

		gpio_request(SABRESD_BT_WAKE, "bt_wake");
		gpio_direction_output(SABRESD_BT_WAKE, 0);
	}
	...
	if (1 == caam_enabled)
		imx6q_add_imx_caam();
#if 0  // remove unused audio codec devices
	if (board_is_mx6_reva()) {
    ...
    // wm8326 PMIC
    if (mx6q_hdmidongle_init_wm8326())
        printk(KERN_ERR "Failed to register wm8326 PMIC i2c device\n");
	...
	*/
#if 1  // do not disable, will not boot
    ...
    //enable adv7180
    gpio_request(MX6DL_PAD_CSIO_PWRDWN, "tvin-pwrdwn");
    gpio_direction_output(MX6DL_PAD_CSIO_PWRDWN, 0);

    gpio_request(MX6DL_PAD_CSIO_RESET, "tvin-reset");
    gpio_direction_output(MX6DL_PAD_CSIO_RESET, 0);
}
